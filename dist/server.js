...
mongoose.connect("mongodb+srv://admin:admin@cluster0.n5hzfrg.mongodb.net/evomed?retryWrites=true&w=majority&appName=Cluster0", {
...
bot.onText(/\/start/, async msg => {
  ...
  bot.sendMessage(msg.chat.id, "👩‍⚕️ <b>EVOMED</b> ga xush kelibsiz 🎉", {
    reply_markup: {
      ...
      keyboard: [["📃 Savol berish", "✍️ Takliflar"], ["✍️ E'tirozlar", "📝 Qabulga yozilish"], ["📍 Lokatsiya", "💊 Xizmatlar"]]
    },
    parse_mode: "HTML"
  });
});
bot.on("message", async msg => {
  ...
  if (user && user.type == "numberForQuestion") {
    ...
    bot.sendMessage("-1001936206921", `<b>Botdan Xabar</b>
...
Xabar ${user.text}`, {
  ...
  if (user && user.type == "number") {
    const qabul = await Qabul.findOne({
      id: msg.chat.id
    });
    bot.sendMessage("-1001936206921", `<b>Botdan Xabar [Qabulga yozilishdi]</b>
<b>📌 ${qabul.doctor}\n⏰ ${qabul.time}\n📅 ${qabul.date}\n👤 ${msg.from.first_name}\n📞${msg.text}</b>`, {
      parse_mode: "HTML"
    });
  ...
  if (msg.text == "📝 Qabulga yozilish") {
    bot.sendMessage(msg.chat.id, `<b>Qabulga yozilmoqchi bo'lgan shifokoringizni tanglang</b>`, {
      parse_mode: "HTML",
      reply_markup: {
        inline_keyboard: [[{
          text: "📌 Pediatr",
